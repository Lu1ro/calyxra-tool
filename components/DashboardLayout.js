...
function isActive(item, pathname) {
    if (item.exact) return pathname === item.href || pathname.startsWith('/dashboard/stores');
                    ...
                    return (
                        <Link key={item.label} href={item.href} legacyBehavior>
                            <a className={`cx-nav-item ${active ? 'cx-nav-item-active' : ''}`} title={item.label}>
                                ...
                                {item.comingSoon && (sidebarOpen || isMobile) && <span className="cx-coming-soon">Soon</span>}
                            </a>
                        </Link>
                    );
                })}
